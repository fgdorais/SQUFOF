  ...
  do {                                                                  \
    if (__builtin_constant_p (bhi) && (bhi) == 0)			\
      __asm__ ("{a%I4|add%I4c} %1,%3,%4\n\t{aze|addze} %0,%2"		\
	     : "=r" (rhi), "=&r" (rlo)                                  \
             : "r" (ahi), "%r" (alo), "rI" (blo));                      \
    else if (__builtin_constant_p (bhi) && (bhi) == ~(USItype) 0)	\
      __asm__ ("{a%I4|add%I4c} %1,%3,%4\n\t{ame|addme} %0,%2"		\
	     : "=r" (rhi), "=&r" (rlo)                                  \
             : "r" (ahi), "%r" (alo), "rI" (blo));			\
    else								\
...
#define subd_u32(rhi, rlo, ahi, alo, bhi, blo) \
  do {									\
    if (__builtin_constant_p (ahi) && (ahi) == 0)			\
      __asm__ ("{sf%I3|subf%I3c} %1,%4,%3\n\t{sfze|subfze} %0,%2"	\
	       : "=r" (rhi), "=&r" (rlo)                                \
               : "r" (bhi), "rI" (alo), "r" (blo));			\
    else if (__builtin_constant_p (ahi) && (ahi) == ~(USItype) 0)	\
      __asm__ ("{sf%I3|subf%I3c} %1,%4,%3\n\t{sfme|subfme} %0,%2"	\
	       : "=r" (rhi), "=&r" (rlo)                                \
               : "r" (bhi), "rI" (alo), "r" (blo));			\
...
#define mulh_u32(rhi,op1,op2) \
  do {									\
    __asm__ ("mulhwu %0,%1,%2" : "=r" (rhi) : "%r" (op1), "r" (op2));	\
  } while (0)

#define muld_u32(rlo,rhi,op1,op2) \
  do {									\
    uint32_t __muld_op1 = (op1);                                        \
    uint32_t __muld_op2 = (op2);					\
    mulh_u32(rhi,op1,op2);                                              \
